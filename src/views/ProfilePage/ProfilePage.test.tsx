...
describe('Profile Page', () => {
      ...
      expect(getByText(/Thank you for being part of our Castrol Community!/i)).toBeInTheDocument();
      //all input fields
      const emailInput = getByRole('textbox', { name: 'Email ID *' });
      const userOldPasswordInput = getByLabelText('Old Password*') as HTMLInputElement;
      const userPasswordInput = getByLabelText('New Password*') as HTMLInputElement;
      const userConfirmPasswordInput = getByLabelText('Confirm New Password*') as HTMLInputElement;
    ...
    test('Validation check of profile page', async () => {
      ...
      const userOldPasswordInput = getByLabelText('Old Password*') as HTMLInputElement;
      fireEvent.change(userOldPasswordInput, { target: { value: '@Bpcastrol2001' } });
      const userPasswordInput = getByLabelText('New Password*') as HTMLInputElement;
      ...
      const userConfirmPasswordInput = getByLabelText('Confirm New Password*') as HTMLInputElement;
      fireEvent.change(userConfirmPasswordInput, { target: { value: '@Bpcastrol2024' } });
    ...
    test("Successfully changes the password of user", async ()=>{ 
      ...
      const userPasswordInput = getByLabelText('New Password*') as HTMLInputElement;
      fireEvent.change(userPasswordInput, { target: { value: '@Bpcastrol2024' } });
      ...
      fetchMock.mockResponseOnce(JSON.stringify({ output: 'success', msg: 'Password was changes successfully' }), { status: 200 });
      ...
      await waitFor(() => {
        ...
        expect(fetch.mock.calls[1][0]).toEqual('http://localhost:3000/user/changepassword'); // Expecting change password endpoint to be called
        expect(sendAlert).toHaveBeenCalledWith({ message: 'Password Changed Successfully', type: 'success' });
    ...
    fetchMock.mockResponseOnce(JSON.stringify({ output: 'fail', msg: 'Internal Server Error' }), { status: 500 });
    ...
    await waitFor(() => {
      expect(sendAlert).toHaveBeenCalledWith({ message: 'Internal Server Error', type: 'error' });
      })
    await waitFor(() => {
    const history = createMemoryHistory({ initialEntries: ['/profile'] }); // Set the initial path
    history.listen(() => {
    ...
    const userOldPasswordInput = getByLabelText('Old Password*') as HTMLInputElement;
    fireEvent.change(userOldPasswordInput, { target: { value: '@Bpcastrol2024' } });
    ...
    fetchMock.mockResponseOnce(JSON.stringify({ output: 'error', msg: 'Old password does not match' }), { status: 201 });

    fireEvent.click(changePassword);

    await waitFor(() => {
      expect(fetch.mock.calls.length).toEqual(2); 
      expect(fetch.mock.calls[1][0]).toEqual('http://localhost:3000/user/changepassword'); 

    });
    await waitFor(() => {
      expect(sendAlert).toHaveBeenCalledWith({ message: 'Old password does not match', type: 'error' });
      })
    await waitFor(() => {
    const history = createMemoryHistory({ initialEntries: ['/profile'] }); // Set the initial path
    history.listen(() => {
    ...
    fetchMock.mockResponseOnce(JSON.stringify({ output: 'error', msg: 'User data was not found' }), { status: 201 });
    ...
    await waitFor(() => {
      expect(sendAlert).toHaveBeenCalledWith({ message: 'User data was not found', type: 'error' });
      })
    await waitFor(() => {
    const history = createMemoryHistory({ initialEntries: ['/profile'] }); // Set the initial path
    history.listen(() => {
    ...
    test("User data not found error for profile page", async ()=>{ 
      fetchMock.mockResponseOnce(JSON.stringify({
        ...
        msg: 'User data was not found',}), { status: 201 });
...
await waitFor(() => {
  const userMobileInput = getByLabelText('Mobile Number*') as HTMLInputElement;
